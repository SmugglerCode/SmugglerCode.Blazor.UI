...
public partial class IntTextBox : DisabledScopeBase
{
    ...
    /// </summary>
    [Parameter]
    public EventCallback<int> OnEnter { get; set; }

    /// <summary>
    /// Callback triggeren when clicking on the self set icon <see cref="Icon"/>
    /// </summary>
    [Parameter]
    public EventCallback<int> OnIconPressed { get; set; }
    ...
    private int ParseValue(object? val)
    {
        if (val == null) return 0;

        return int.TryParse(val.ToString(), out var value) ? value : 0;
    }
